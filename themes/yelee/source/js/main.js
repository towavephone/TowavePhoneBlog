require([], function (){
    ...
    // PC
    $("#container .left-col .overlay").css({"background-color": colorList[id]});
    // Mobile
    $("#container #mobile-nav .overlay").css({"background-color": colorList[id]});
    ...
    if(yiliaConfig.isArchive || yiliaConfig.isTag || yiliaConfig.isCategory) {
        $(document).ready(function() {
            $("#footer").after("<button class='hide-labels'>标签切换</button>");
